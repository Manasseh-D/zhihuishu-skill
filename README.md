...
1. **模型选择**：推荐使用 200K 左右上下文的模型（需触发 auto compacting）。超长上下文在后期会不受控制、表现变差、花费剧增。推荐模型：**DeepSeekV4-Flash**，便宜量大（默认 200K 上下文）

2. **重要文件**：运行过程会产生两个重要文件：
   - `题库.md`：CC 在运行过程中会将测试题目全量记录，跑完一个课程后获得的题库可**相互分享**，提高他人运行效率并节省花费
   - `progress.md`：记录任务完成进展

3. **运行稳定性**：任务完成时间随任务量增加而延长。MCP、网络、CC 偶有 bug 或页面加载异常，建议偶尔检查一下，必要时手动刷新浏览器
...
4. 笔者非专业项目开发者，skill 中应还有诸多无力改进之处，只希望能帮助大家节省更多精力，专注医学学习
